...
(define (enqueue x q)
  (if (= (queue-size-l q) (queue-size-r q))
      (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)]
                   [right '()]
...
(define (dequeue q)
    (if (= (queue-size-l q) (queue-size-r q))
      (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)]
                   [right '()]
                   [size-l (sub1 (+ (queue-size-l q) (queue-size-r q)))]
                   [size-r 0])
      (struct-copy queue q [left (stream-rest (queue-left q))]
